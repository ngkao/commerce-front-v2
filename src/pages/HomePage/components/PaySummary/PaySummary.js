...
const PaySummary = ({
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      ...
      if (showQR === true) {
        axios
          .get(`${REACT_APP_SERVER_URL}/timestamp`)
          .then((latestData) => {
            if (latestOrderData === null) {
              setLatestOrderData(latestData.data);
            } else if (
              latestData.data.created_at > latestOrderData.created_at
            ) {
              setLatestOrderData(latestData.data);
              successPayment(); // eslint-disable-next-line
            }
          })
  ...
  useEffect(() => {
    setTimeout(() => {
      ...
      items.forEach((item) => item.classList.add("checkout__qr--active"));
    }, 300);
  }, [showQR]);
  ...
  useEffect(() => {
    ...
    items.forEach((item) =>
      item.classList.remove("checkout__totalpay-amount--top--final")
    );
    //Bottom
    const itemsBottom = document.querySelectorAll(
      ".checkout__totalpay-amount--bottom"
    );
    itemsBottom.forEach((item) =>
      item.classList.remove("checkout__totalpay-amount--bottom--final")
    );
    setTimeout(() => {
      //Top
      const items = document.querySelectorAll(
        ".checkout__totalpay-amount--top"
      );
      items.forEach((item) =>
  ...
  return (
    <div className="checkout">
      {showQR ? (
        <a
          className="checkout__qr-link"
          href={urlStripe}
          target="_blank"
          rel="noreferrer"
        >
          <img className="checkout__qr" src={src} alt="Checkout" />
        </a>
      ) : null}
      {successPtmAlert ? (
        <div className="checkout__success-ctr">
          <div className="checkout__lottie">
            <Lottie loop={false} autoplay={true} animationData={SuccessMark} />
          </div>
          <p className="checkout__success-msg">
            {latestOrderData.customer_name}
          </p>
