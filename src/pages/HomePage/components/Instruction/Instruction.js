...
const Instruction = () => {
  ...
  const navItems = document.querySelectorAll(".nav__item");   // eslint-disable-next-line
  const navItemProducts = Array.from(navItems).find(
    (item) => item.textContent.trim() === "Products"
  );   // eslint-disable-next-line
  const navItemInventory = Array.from(navItems).find(
    (item) => item.textContent.trim() === "Inventory"
  );   // eslint-disable-next-line
  const navItemSales = Array.from(navItems).find(
    (item) => item.textContent.trim() === "Sales History"
  );   // eslint-disable-next-line
  const navItemInsights = Array.from(navItems).find(
    (item) => item.textContent.trim() === "Insights"
  );
  const paysumCoverBottom = document.querySelector(".pay-cover-bottom");
  const paysumCoverTop = document.querySelector(".pay-cover-top");   // eslint-disable-next-line
      ...
      <div className={`instruction__ctr`}>
        <div className="instruction__subctr">
          ...
          <div className="instruction__btn-ctr">
            <p className="instruction__skip" onClick={handleSkip}>
              SKIP
            </p>
            <p className="instruction__next" onClick={handleNextCard}>
              {cardNumber === 0
                ? "LEARN"
                : cardNumber === 5
                ? "GET STARTED"
